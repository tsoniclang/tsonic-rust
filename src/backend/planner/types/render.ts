...
function collectAliasesFromRustGenericArgument(
  argument: RustAstGenericArgument,
  register: (path: string) => void,
): void {
  if (argument.kind === "type") collectAliasesFromRustType(argument.type, register);
  else if (argument.kind === "associated-equality") {
    collectAliasesFromRustType(argument.type, register);
    for (const nested of argument.genericArguments ?? []) collectAliasesFromRustGenericArgument(nested, register);
...
function collectAliasesFromRustTypeBound(
  ...
  if (bound.kind === "trait") collectAliasesFromRustType(bound.trait, register);
    ...
    bound.captures.forEach((capture) => collectAliasesFromRustGenericArgument(capture, register));
  }
}
