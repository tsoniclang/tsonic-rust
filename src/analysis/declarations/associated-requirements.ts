...
export function createRustAssociatedRequirementCollector(
  ...
  const entries: { carrier: Extract<TargetTypeRef, { readonly kind: "associated-type" }>; requirements: Set<RustGenericRequirement> }[] = [];
  const require = (carrier: Extract<TargetTypeRef, { readonly kind: "associated-type" }>, requirement?: RustGenericRequirement): boolean => {
    if (carrier.trait?.sourceItem === undefined || families.get(carrier.trait.id) === undefined) return false;
    const references = rustTargetTypeParameterNames(carrier);
    if (references.length === 0) {
      const implementation = families.implementation(carrier.trait.id, carrier.owner);
    ...
    if (entry === undefined) {
      entry = { carrier, requirements: new Set() };
      entries.push(entry);
    }
  ...
  const collect = (carrier: TargetTypeRef): boolean => {
    if (carrier.kind === "associated-type" && carrier.trait?.sourceItem !== undefined && !require(carrier)) return false;
