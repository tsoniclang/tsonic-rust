...
import { rustScreamingSnakeIdentifier } from "../../target-model/names/identifiers.js";
...
export interface RustClassValueDefinition {
  ...
  readonly views: readonly (RustClassValueView & { readonly storageName: string })[];
}
...
export function createRustClassValueRegistry(): RustClassValueRegistry {
  ...
  return {
    ...
    seal(context) {
      ...
      const { ast } = context;
      const allocate = (declaration: Node, purpose: string): string => {
        ...
        const base = rustScreamingSnakeIdentifier(`${context.names.nameForDeclaration(declaration)}_${purpose}`);
        let selected = base;
        let suffix = 2;
...
export function resolveRustClassValue(
  ...
  const reject = (): undefined => {
    appendRustDiagnostic(walk, "RUST_CLASS_VALUE_NOT_CLOSED",
      "Class constructor values require exact same-component static data-property correspondence and closed structural storage.",
  ...
  for (const field of shape.fields) {
    ...
    if (pair?.kind !== "present" || pair.source.read !== "property" || field.method === true ||
  ...
  if (!walk.context.classValues.record({ declaration, carrier, fields })) return reject();
