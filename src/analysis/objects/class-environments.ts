...
export function selectRustClassEnvironment(walk: RustFactWalk, declaration: Node): RustClassEnvironmentSelection {
  ...
  if (definition?.kind !== "class" || ast.parent(declaration) === ast.getSourceFile(declaration)) return { kind: "none" };
  ...
  const selected = collectRustLexicalCaptures(walk, declaration, captureRoots);
  ...
  for (const member of ast.members(declaration)) {
    ...
    if (!ast.hasModifierKind(member, "static") || sourceClassFieldIsTypeOnly(ast, member) ||
      ast.kindName(member) !== "KindPropertyDeclaration") continue;
  ...
  if (captures.length === 0 && staticFields.length === 0) return { kind: "none" };
  ...
  const copy = staticFields.length === 0 && captures.every(capture => capture.storage === "value" && isRustCopyCarrier(capture.carrier));
  return { kind: "available", environment: Object.freeze({ declaration, carrier: projectTypes.openCarrier(definition),
    storage: copy || staticFields.length === 0 && captures.length === 1 && captures[0]!.storage === "location" ? "value" : "shared",
    copy,
