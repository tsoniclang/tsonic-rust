...
import type { RustSourceObjectField } from "../../project-types/source-type-registry.js";
import type {
  RustArgumentMode,
...
export function mapSelectedStringRegExpProtocolCall(
  ...
  const protocolFieldMatches = protocolShape?.fields.filter((field) =>
    ...
    fieldSelectsProtocolSymbol(field, selection.symbol, context)) ?? [];
  const protocolField = protocolFieldMatches.length === 1 ? protocolFieldMatches[0] : undefined;
  if (protocolField === undefined) {
    return undefined;
  }
  const actualCallableCarrier = rustStructuralMethodCallableCarrier(
...
function fieldSelectsProtocolSymbol(
  field: RustSourceObjectField,
  symbol: RegExpProtocolSymbol,
  context: RustOperationPolicyContext,
): boolean {
  return field.declarations.some((declaration) => {
    const name = context.ast.name(declaration);
    return name !== undefined && context.ast.is.IsComputedPropertyName(name) &&
      context.semanticsFor(declaration).operations.wellKnownSymbol(name)?.kind === symbol;
  });
}
