...
export function rustTypeFamilyNormalizer(
  ...
  const normalize = (carrier: TargetTypeRef): TargetTypeRef => {
    if (carrier.kind !== "associated-type" || carrier.trait?.sourceItem === undefined) return carrier;
    const selected = families.implementation(carrier.trait.id, carrier.owner);
    if (selected === undefined) return carrier;
    const identity = closedMetadataKey({ family: carrier.trait.id, owner: carrier.owner });
