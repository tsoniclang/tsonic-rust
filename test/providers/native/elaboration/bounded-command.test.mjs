...
const protocolRoot = new URL("../../../../dist/providers/native/protocol/", import.meta.url);
...
function sharedInput(maximumDiagnosticBytes = 16) {
  return {
...
async function fixtureRunner(setup, replacement = false) {
...
function isolated(source, workerData = {}) {
...
function expectDead(identities) {
...
function cleanRecordedProcesses(path) {
...
function descendantSource(path, {
