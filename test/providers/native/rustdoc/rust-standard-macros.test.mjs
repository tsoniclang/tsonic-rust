...
import { vec as list } from "@tsonic/rust/std/index.js";
import { vec as allocated } from "@tsonic/rust/alloc/index.js";
import { tokens } from "@tsonic/rust/lang.js";
function identity(value: Vec<int32>): Vec<int32> { return value; }
export function main(): void {
  const empty: Vec<int32> = list([]);
  const values = identity(list([1, 2, 3]));
  const count: nativeUint = 3;
  const copies: Vec<int32> = list(tokens\`[7; \${count}]\`);
  const inferred = list([2, 3]);
  const alias: Vec<int32> = allocated([4]);
  let calls: int32 = 0;
  const zero: Vec<int32> = list(tokens\`[\${++calls}; 0]\`);
  const strings = list(tokens\`["value"; 2]\`);
...
test("native checking rejects runtime formats and incompatible contextual vector elements", () => {
...
` } });
  ...
  assert.ok(result.diagnostics.some(diagnostic => diagnostic.category === "error" && /format argument must be a string literal/u.test(diagnostic.message)));
  assert.throws(() => compileRustThroughTargetPack({ files: { "index.ts": `
import type { int32 } from "@tsonic/core/types.js";
import { vec } from "@tsonic/rust/std/index.js";
import type { Vec } from "@tsonic/rust/std/vec.js";
export function bad(): Vec<int32> { return vec(["wrong"]); }
` } }), /not assignable/u);
});

test("the superseded vector repetition facade is not a native export", () => {
  assert.throws(() => compileRustThroughTargetPack({ files: { "index.ts": `
import { vecRepeat } from "@tsonic/rust/std/vec.js";
export const values = vecRepeat(1, 2);
` } }), /vecRepeat/u);
});
