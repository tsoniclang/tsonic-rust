...
const sourceFacts = readFileSync(
  new URL("../../src/policy/safety/source-explicit-safety.ts", import.meta.url),
  "utf8",
);
const nativePlanner = readFileSync(
  new URL("../../src/backend/planner/expressions/native-pointers.ts", import.meta.url),
  "utf8",
);
const safetyPlanner = readFileSync(
  new URL("../../src/backend/planner/safety/explicit-safety.ts", import.meta.url),
  "utf8",
);
...
test("shared explicit-safety fact keys have one Rust source-boundary reader", () => {
  ...
  for (const key of [
    ...
    assert.deepEqual(
      ...
      ["src/policy/safety/source-explicit-safety.ts"],
      key,
    );
  }
});
