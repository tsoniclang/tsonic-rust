...
test("delimited postfix operations cannot split a block operand into a statement", () => {
  const value = { kind: "path", path: "value" };
  for (const receiver of [
    { kind: "block", bindings: [], value },
    { kind: "evaluate-then", effect: { kind: "call", path: "effect", args: [] }, discard: "unit", value },
    { kind: "unsafe", expression: value },
    { kind: "conditional", condition: { kind: "bool-literal", value: true }, whenTrue: value, whenFalse: value },
    { kind: "match", expression: value, arms: [{ pattern: { kind: "wildcard" }, expression: value }] },
  ]) {
    const operand = printRustExpr(receiver);
    assert.equal(printRustExpr({ kind: "index", receiver, index: { kind: "int-literal", text: "0" } }), `(${operand})[0]`);
    assert.equal(printRustExpr({ kind: "invoke", callee: receiver, args: [] }), `(${operand})()`);
  }
  assert.equal(printRustExpr({ kind: "index", receiver: value, index: { kind: "int-literal", text: "0" } }), "value[0]");
  assert.equal(printRustExpr({ kind: "invoke", callee: value, args: [] }), "value()");
});
