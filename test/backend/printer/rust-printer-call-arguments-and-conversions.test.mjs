...
test("unary operators and casts preserve their exact AST grouping", () => {
  const value = { kind: "path", path: "value" };
  const cast = { kind: "numeric-cast", target: "i32", expression: value };
  for (const operator of ["!", "-"]) {
    assert.equal(printRustExpr({ kind: "unary", operator, operand: cast }), `${operator}(value as i32)`);
    assert.equal(printRustExpr({ kind: "numeric-cast", target: "i32",
      expression: { kind: "unary", operator, operand: value } }), `${operator}value as i32`);
  }
  assert.equal(printRustExpr({ kind: "numeric-cast", target: "i32",
    expression: { kind: "dereference", pointer: value } }), "*value as i32");
  assert.equal(printRustExpr({ kind: "dereference", pointer: {
    kind: "numeric-cast", target: "*const i32", expression: value,
  } }), "*(value as *const i32)");
});
