...
`,
  ...
  assert.equal(appendOutput.match(/output = js_abi::JsString::concat/gu)?.length, 4);
  assert.match(appendOutput, /js_abi::JsString::from\("\/"\)/u);
  assert.match(appendOutput, /js_abi::JsString::from\("'"\)/u);
  assert.match(appendOutput, /js_abi::JsString::from\("🙂"\)/u);
  assert.doesNotMatch(output, /output\.push(?:_str)?\(/u);
  assert.doesNotMatch(appendOutput, /format!/u);
  assert.match(appendFailureOutput, /output = js_abi::JsString::concat/u);
  assert.doesNotMatch(appendFailureOutput, /format!/u);
  assert.match(selfAppendOutput, /js_abi::JsString::concat/u);
