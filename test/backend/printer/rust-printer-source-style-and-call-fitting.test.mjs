...
test("constant-folded parameter reads retain only the exact unused-binding expectation", () => {
  ...
  for (const used of [false, true]) {
    const callable = { kind: "function", name: "selected", visibility: "crate", generics: emptyRustGenerics,
    ...
    const model = finalizeRustSourceStyle({ headerComment, items: [
      ...
      { kind: "trait", name: "Contract", visibility: "crate", generics: emptyRustGenerics,
        members: [callable, { ...callable, name: "required", body: undefined }] },
    ] });
    const functions = [model.items[0], model.items[1].members[0], model.items[2].members[0]];
    ...
    assert.deepEqual(model.items[2].members[1].attrs ?? [], []);
...
test("Iterator naming expectations require the exact mutable-reference receiver", () => {
  for (const [receiver, parameters, expected] of [
    ...
    assert.equal(model.items[1].members[0].attrs?.includes(rustLintAttributes.shouldImplementTrait) ?? false,
