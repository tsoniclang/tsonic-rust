...
function printRustSourceFile(model, edition = "2021") {
  return printFinalRustSourceFile(finalizeRustSourceStyle(model, edition));
}
...
test("source imports follow the selected Rust edition's canonical ordering", () => {
  const model = {
    headerComment,
    items: [{ kind: "use", path: "crate::index::BuildResult" },
      { kind: "use", path: "crate::index::build_site" },
      { kind: "use", path: "crate::index::main" }],
  };

  assert.match(
    printRustSourceFile(model),
    /use crate::index::build_site;\nuse crate::index::main;\nuse crate::index::BuildResult;/u,
  );
  assert.match(
    printRustSourceFile(model, "2024"),
    /use crate::index::BuildResult;\nuse crate::index::build_site;\nuse crate::index::main;/u,
  );
});
