...
test("constant-folded parameter reads retain only the exact unused-binding expectation", () => {
  ...
  for (const used of [false, true]) {
    ...
    for (const selected of functions) {
      assert.equal(selected.attrs?.some(attribute => attribute.includes("expect(unused_variables")) ?? false, !used);
      assert.deepEqual(selected.params, [parameter]);
    }
    assert.equal(model.items[2].functions[1].attrs?.some(attribute => attribute.includes("unused_variables")) ?? false, false);
