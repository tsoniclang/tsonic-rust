...
test("array repetition retains exact constants and visits its element and named extent", () => {
  const expression = { kind: "array-repeat", element: { kind: "call", path: "values::empty", args: [] },
    length: { kind: "integer", value: 9007199254740993n } };
  const item = value => ({ kind: "function", name: "repeat", visibility: "private", generics: emptyRustGenerics,
    params: [], body: { statements: [{ kind: "tail", expr: value }] } });
  assert.equal(printRustExpr(expression), "[values::empty(); 9007199254740993]");
  assert.deepEqual(rustExpressionChildren(expression), [expression.element]);
  assert.equal(rustItemsReferenceModuleAlias([item(expression)], "values"), true);
  const named = { ...expression, length: { kind: "path", path: "sizes::COUNT" } };
  assert.equal(rustItemsReferenceModuleAlias([item(named)], "sizes"), true);
  assert.deepEqual(rustExpressionChildren(named), [expression.element, { kind: "path", path: "sizes::COUNT" }]);
});
