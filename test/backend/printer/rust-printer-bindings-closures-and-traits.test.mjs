...
test("rustfmt canonicalizes fallible calls with single-expression closures", () => {
  const source = printRustSourceFile({
    headerComment,
    items: [{
      kind: "function",
  ...
  assert.match(
    source,
    /values\n {8}\.try_sort\(\|left, right\| compare_with_a_deliberately_long_checked_operation\(left, right\)\)\?;/u,
  );
  assert.doesNotMatch(source, /\|left, right\| \{/u);
});
