...
test("rustfmt canonicalizes trailing conditional closures", () => {
  const source = printRustSourceFile({
    headerComment,
    items: [{
      kind: "function",
  ...
  assert.match(source, /let total = values\.reduce_with_array\(\n {8}0,/u);
  assert.match(
    source,
    /\|sum, value, current, array\| if current >= 0\.0 \{ sum \+ value \} else \{ sum \},/u,
  );
  assert.doesNotMatch(source, /\|sum, value, current, array\| \{/u);
});
