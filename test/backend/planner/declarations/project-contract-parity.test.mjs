...
`,
  ...
  assert.deepEqual(result.diagnostics, []);
  const source = artifactText(result, "src/index.rs");
  assert.match(source, /let equal: bool = \{\s*let _ = left\(counter\.clone\(\)\);\s*\{\s*\{\s*right\(counter\.clone\(\)\);\s*\};\s*false\s*\}\s*\};/u);
  assert.match(source, /let unequal: bool = \{\s*let _ = left\(counter\.clone\(\)\);\s*\{\s*\{\s*right\(counter\.clone\(\)\);\s*\};\s*true\s*\}\s*\};/u);
