...
test("C-style integer loops lower to native Rust ranges", () => {
  const { result } = compileRust({
    files: {
      "index.ts": `
import type { int32 } from "@tsonic/core/types.js";
...
`,
  ...
  assert.deepEqual(result.diagnostics, []);
  const text = artifactText(result, "src/index.rs");
  assert.match(text, /for i in 0\.\.limit \{/u);
...
`,
  ...
  assert.match(text, /!\(10\.\.=20\)\.contains\(&value\)/u);
  assert.match(text, /value\.partial_cmp\(&10\.0\) == Some\(core::cmp::Ordering::Less\)/u);
  assert.match(text, /value\.partial_cmp\(&20\.0\) == Some\(core::cmp::Ordering::Greater\)/u);
