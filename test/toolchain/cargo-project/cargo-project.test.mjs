...
const noRuntimeReferences = Object.freeze({
  ...
  foundation: "std",
  minimumFoundationByCrate: new Map(),
});

function planCargoManifest(configuration, runtimeReferences) {
  const analysis = analyzeRustRuntimeReferences(runtimeReferences, configuration.foundation);
...
function runtimeReference(crate, path, {
  ...
  return {
    ...
    attributes: {
      crate,
      ...(registryPatch === undefined ? {} : { registryPatch }),
      ...(minimumFoundation === undefined ? {} : { minimumFoundation }),
      ...(defaultFeatures === undefined ? {} : { defaultFeatures: String(defaultFeatures) }),
...
test("core, alloc, and std foundations emit exact crate roots and Cargo features", () => {
  for (const [foundation, feature, rootLines] of [
    ["core", undefined, ["// Generated by the Tsonic Rust target. Do not edit.", "#![no_std]", ""]],
    ["alloc", "alloc", [
      "// Generated by the Tsonic Rust target. Do not edit.",
      "#![no_std]",
      "",
    ...
    const result = compile(fakeCompileInput({
      target: { id: "rust", options: { foundation } },
      sourceFiles: [fakeSourceFile({ fileName: "src/empty.ts", text: "", statements: [] })],
      runtimeReferences: [runtimeReference(
        "tsonic_rust_runtime",
        runtimeCrate,
    ...
    assert.deepEqual(result.diagnostics, []);
    const manifest = result.artifacts.find((artifact) => artifact.path === "Cargo.toml").text;
    assert.match(manifest, /default-features = false/u);
    if (feature === undefined) {
      assert.doesNotMatch(manifest, /, features =/u);
    } else {
      assert.match(manifest, new RegExp(`features = \\["${feature}"\\]`, "u"));
    }
    const root = result.artifacts.find((artifact) => artifact.path === "src/lib.rs").text;
    assert.deepEqual(root.split("\n").slice(0, rootLines.length), rootLines);
  }
});

test("foundation requirements fail closed before Cargo", () => {
  const result = compile(fakeCompileInput({
    target: { id: "rust", options: { foundation: "core" } },
    runtimeReferences: [runtimeReference("hosted_provider", materializedCrate("hosted_provider"))],
  }));
  assert.equal(result.artifacts.length, 0);
  assert.equal(result.diagnostics[0].code, "RUST_FOUNDATION_REQUIREMENT_UNSATISFIED");
  assert.match(result.diagnostics[0].message, /requires 'std'.*selected 'core'/u);
});

test("Cargo feature selections are exact dependency identity", () => {
  const first = runtimeReference("tsonic_rust_runtime", runtimeCrate, {
    minimumFoundation: "core",
    defaultFeatures: false,
    features: ["alloc"],
  });
  const plan = planCargoManifest(
    { ...generatedConfiguration, foundation: "alloc" },
    [first, first],
  );
  assert.deepEqual(plan.diagnostics, []);
  assert.equal(
    printCargoManifest(plan.manifest).includes(
      'default-features = false, features = ["alloc"]',
    ),
    true,
  );

  const conflict = planCargoManifest(
    { ...generatedConfiguration, foundation: "alloc" },
    [first, runtimeReference("tsonic_rust_runtime", runtimeCrate, {
      minimumFoundation: "core",
      defaultFeatures: false,
      features: [],
    })],
  );
  assert.equal(conflict.manifest, undefined);
  assert.equal(conflict.diagnostics[0].code, "RUST_INVALID_CARGO_REFERENCE");

  const foundationConflict = planCargoManifest(
    { ...generatedConfiguration, foundation: "alloc" },
    [first, runtimeReference("tsonic_rust_runtime", runtimeCrate, {
  ...
  assert.equal(foundationConflict.manifest, undefined);
  assert.equal(
    foundationConflict.diagnostics[0].code,
    "RUST_INVALID_CARGO_REFERENCE",
  );
});
